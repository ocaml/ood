...
module Academic_institution = struct
  ...
  type t = [%import: (Ood.Academic_institution.t[@with Ood.Academic_institution.location := location])] [@@deriving yaml]
  ...
  let widget_of_t =
    Widget.
      [
        ...
        `String (String.make ~required:true ~label:"Continent" ~name:"continent" ());
  ...
  let folder =
    ...
    Netlify.Collection.Folder.make ~name:"academic_institution" ~create:true
      ~format:Yaml_frontmatter ~label:"Academic Institution" ~folder:path ~fields ()
end
